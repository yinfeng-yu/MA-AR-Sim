...
public class HorizontalOrbitalSolver : Solver
{
    [SerializeField]
    [Tooltip(
    ...
    [SerializeField]
    [Tooltip(
        "XYZ offset for this object oriented with the TrackedObject/TargetTransform's forward. Mixing local and world offsets is not recommended. Local offsets are applied before world offsets.")]
    ...
    [SerializeField]
    [Tooltip(
        "XYZ offset for this object in worldspace, best used with the YawOnly orientationType. Mixing local and world offsets is not recommended. Local offsets are applied before world offsets.")]
    ...
    {
        Vector3 desiredPos = SolverHandler.TransformTarget != null
            ? SolverHandler.TransformTarget.position
            : Vector3.zero;

        Quaternion targetRot = SolverHandler.TransformTarget != null
        ...
        ///Only lines changed from the MRTKK Orbital Solver
        GoalPosition = new Vector3(desiredPos.x, transform.position.y, desiredPos.z);
        Vector3 currentRot = transform.rotation.eulerAngles;
        Vector3 desiredRotEuler = desiredRot.eulerAngles;
        GoalRotation = Quaternion.Euler(new Vector3(currentRot.x, desiredRotEuler.y, currentRot.z));
    }
    ...
    private Quaternion CalculateDesiredRotation(Vector3 desiredPos)
    {
        ...
        switch (orientationType)
        {
            case SolverOrientationType.YawOnly:
                float targetYRotation = SolverHandler.TransformTarget != null
            ...
            case SolverOrientationType.CameraFacing:
                desiredRot = SolverHandler.TransformTarget != null
                    ? Quaternion.LookRotation(CameraCache.Main.transform.position - desiredPos)
                    : Quaternion.identity;
                break;
